...
def _try_anthropic(explicit_api_key: str = None) -> Tuple[Optional[Any], Optional[str]]:
    ...
    is_direct_anthropic = _is_anthropic_compatible_host(base_url)
    if auth_mode == ANTHROPIC_AUTH_MODE_SUBSCRIPTION_ONLY and is_direct_anthropic:
        pool_present = False
        entry = None
        try:
            token = resolve_anthropic_credentials(
                auth_mode=auth_mode,
                explicit_api_key=explicit_api_key,
            ).token
        except Exception as exc:
    ...
    base_url = _pool_runtime_base_url(entry, base_url) if pool_present else base_url
