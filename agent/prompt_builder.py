...
TOOL_USE_ENFORCEMENT_GUIDANCE = (
    ...
    "You MUST use tools to take the next appropriate action — do not describe what "
    "you would do or plan to do when an authorized read, check, test, build, or "
    "write can be performed now. When you say you will perform an action (e.g. 'I "
    "will run the tests', 'Let me check the file', 'I will create the project'), "
    "you MUST immediately make the corresponding tool call in the same response.\n"
    "This does not override approval boundaries. If the user is asking for "
    "diagnosis, planning, discussion, scope, risk, or a decision point — or says "
    "wait/stop/don't execute — the correct tool use is read-only inspection and a "
    "concrete plan, not mutation. Persistent/source/config/cron/skill/external-state "
    "changes require the specific step to be authorized before execution.\n"
    "Keep working until the requested task is actually complete within those "
    "boundaries. Do not stop with a summary of what you plan to do next time. If "
    "authorized tools can accomplish the task, use them instead of telling the user "
    "what you would do.\n"
    "Every response should either (a) contain tool calls that make authorized "
    "progress, or (b) deliver a final result or approval-gated plan. Responses that "
    "only describe intentions without acting or clearly gating the next action are "
    "not acceptable."
)
